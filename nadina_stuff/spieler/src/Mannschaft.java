...
public class Mannschaft {
  ...
  /**
   * array durch Vector ersetzt, old -> private Spieler[] alleSpieler;
   * */
  ...
  */
  public Mannschaft(String name, Kategorie kategorie) {
    ...
    //
    this.alleSpieler = new Vector<Spieler>(MAX_SPIELER_PRO_TEAM);
    ...
    if (spielerCounter < MAX_SPIELER_PRO_TEAM) {
      ...
      if (spieler.getIstLizenziert()) {
        if (lizenzierteSpieler < kategorie.getMaxAnzahlLizenzierteSpieler()) {
          //ok noch möglich
          alleSpieler.set(spielerCounter++, spieler);
          lizenzierteSpieler++;
          return true;
        } // end of if
        else
  ...
  */
  public void removeSpieler(Spieler spieler) {
    ...
    for (int i=0; i < spielerCounter; i++) {
      if (alleSpieler.get(i) != spieler) {
        // umkopieren
        tmp[tmpCount++] = alleSpieler.get(i);
      } // end of if
    } // end of for
